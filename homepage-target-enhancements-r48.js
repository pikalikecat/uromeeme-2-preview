...
(() => {
 ...
 const messages=["📢  歡迎肛門直腸外科 周杰倫 醫師加入津久診所醫療團隊💯","📢  寒假期間，本院手術量較大，請盡早預約😇","📢  本院正在招募GG增大手術素人，歡迎加入官方 LINE 諮詢"], reduced=matchMedia('(prefers-reduced-motion: reduce)');
 if(copy){
 const sizing=document.createElement('div');
 sizing.className='announcement-sizing-r48';sizing.setAttribute('aria-hidden','true');
 messages.forEach(message=>{const line=document.createElement('span');line.textContent=message;sizing.appendChild(line);});
 copy.parentElement.appendChild(sizing);
 ...
 if(!prev||!nextButton||!track.querySelector('article')){controls.hidden=true;return;}
